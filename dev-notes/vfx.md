...
## Spawn flash: planting, building, discovering (2026-09-12)

`SpawnFlash` (`scripts/vfx/spawn_flash.gd`) is the one bright moment an action gets. **Planting / building**: the thing ARRIVES as a dithered gradient at its real colours, sweeping from the cell nearest the player to the furthest (a fence run grows away from you, a ladder climbs, a plant grows away from your feet), turns flat white the moment it is whole, and the white fades off over 0.9 s (arrival is 0.4 s + 0.08 s per cell). **Discovery**: the plant fades to gold over 0.225 s, holds 0.05 s, fades back over 0.45 s; a plain fade, no dither, and half the build tempo, by request. Preview: `preview_spawn_flash.gd` (rendering context), one strip per subject, `--species <id>`; it exits 1 if a row never moves toward its flash colour or if an overlay node / a flashing plant outlives its tween.

### What is dithered and what is not

The palette rule (CLAUDE.md) locks authored RGB to palette2. The ARRIVAL respects it: `flash_common.gdshaderinc` draws a texel either fully or not at all, chosen by a 4x4 Bayer threshold in texel space (not `FRAGCOORD`: the integer upscale is per monitor) offset by the fragment's progress along `reveal_from -> reveal_to` in world space, with a soft band `reveal_band` wide; `reveal_amount` sweeps it. The FLASH does not: `flash_color` is composited over the sprite at `flash_amount` opacity, a plain mix, which is exactly a white (or gold) copy at alpha t over the art, and alpha is free under the rule. An earlier cut dithered the flash too; the smooth fade was chosen over it deliberately. Colours: `Palette.TEXT` (P23) for built/planted, `Palette.ACCENT` (P12, the unread-inspect gold) for discovery. Cost: one CanvasItem per painted cell for ~1.5 s with six integer ops per fragment; not measurable against this project's noise floor, and the real cost (the cell erase + repaint dirtying the layer's quadrant twice) is the same for any fade-in that has to hide a painted tile.

### Two hosts, because plants and structures are different things

- **Plants carry the flash in their own material.** `wind.gdshader` includes the file, discards unrevealed fragments and applies `_flash` on its final sample, so a swaying plant keeps swaying while it flashes (defaults reveal 1 / flash 0 = inert). `Frailejon._begin_flash` DUPLICATES the species' shared sway material per plant for the length of the tween and hands the shared one back in `_end_flash`; a species with no sway material gets a bare `flash.gdshader`. No instance uniforms (the 4096-pool trap). Burning wins: `apply_burn_material` kills a running flash first, and `_end_flash` only restores over a material that is still its own. `test_spawn_flash.gd` pins the handshake.
- **Structures are tiles on a shared layer**, one material per layer and no way to hide a cell, so each painted cell gets a `SpawnFlash` node under its layer that ERASES the cell, draws the tile's atlas region itself, and REPAINTS the cell when the tween ends or on leaving the tree early (only if the cell is still bare: a fence turned by a neighbour's build knows better). Every node of one construction shares ONE material and ONE tween, so the gradient runs across the whole thing; `flash_built` takes the traversals and the first-clicked cell. A fence RUN is flashed after every fence is down, since each build turns its neighbours' art. A `Pathfinder.graph_changed` during the flash sets a flag and the repaint is followed by one `rebuild()`, because a rebuild while a bridge deck's cell is bare would drop it from the grid.

### Two things measured on the way, both of which the fire overlay gets differently

1. **Tile placement.** A tile's texture is centred on `map_to_local(cell)` and pushed by `-texture_origin`; nothing else. `BurningCellVFX._spawn_overlay` adds `+ tile_size / 2` on top of that, and an overlay placed by its formula rendered a ghost fence half a cell right and down of the real one (probe on `tileset_test`, cell (-2, -20)). Whether the burning-grass overlay is visibly offset in the game was NOT checked; it is flagged, not fixed.
2. **A child of a y-sorted `TileMapLayer` does not inherit the layer's `y_sort_origin`.** The tile sorts at cell y + the layer's origin (`alt * 8` on the structure layers) + the TileData's origin (-16 on a fence); a child node sorts at its own y and nothing else. At altitude 12 an overlay at plain cell y was invisible at every shift from -34 to +95 and fully visible from exactly +96 on. `SpawnFlash` therefore adds both origins plus `SORT_EPS` (0.5) to its position and draws the art back by the same amount. The same arithmetic applies to `BurningCellVFX` on any ground layer above altitude 0 (their origins are `alt * 16`); its flames carry `z_index = 1`, which may be why nobody noticed.

### Also measured

- In a canvas_item `fragment()`, the incoming `COLOR` is already vertex colour x `TEXTURE`. Multiplying by it again squares the texel and tints the flash by it; `flash.gdshader` reads the modulate in `vertex()` into a varying, as `wind.gdshader` does.
- `benchmark_wind_plant.gd --verify` fails on the current tree with or without this change (checked by stashing the shader edit, twice): *E. hartwegiana*, *Chusquea*, *Hypericum* and *Arcytophyllum* are STATIC, *Cortaderia* moves. Their authored strengths (0.3-0.5x `wind_heavy`) sit under the ~0.6x `round()` floor this file already documents. Not addressed here.
