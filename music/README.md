...
`xml_to_strudel.py` maps the **whole piece → 1 Strudel cycle** and emits every
note as `pitch@quarterLength`, so durations come straight from the score. Tempo →
`setcpm(bpm / totalQuarters)` (meter-agnostic; time signatures are ignored, and
the per-measure line breaks in the output are cosmetic). Each part becomes its
own `note(...)` (or `sound(...)` for drums) layer in a `stack(...)`.
...
| @strudel/core, @strudel/mini 1.1.0 | npm | repo → codeberg.org/uzu/strudel (the GitHub repo is archived/moved) |
...
# NOTE: this one-liner extracts only the FIRST note(`...`) layer — it doesn't
# check the other stacked layers, and throws on a drum-only output (no note()).
...
- `*.strudel.js` — arrangement sources (committed). Tune them here; each is
  paste-compatible with strudel.cc.

## Runtime — what the game actually plays

`music/` is the authoring + conversion workspace; it is **not** what the game
loads. The game fetches a GENERATED copy under `docs/music/` (the web-export
root). Promote a tuned arrangement into the build with the sync tool — it copies
the `PUBLISHED` songs and prepends a "do not edit" banner:

```bash
# from the project root:
"../Godot_v4.6.1-stable_win64.exe/Godot_v4.6.1-stable_win64_console.exe" --path . --headless --script res://scripts/tools/sync_music.gd
```

Add a song to `scripts/tools/sync_music.gd`'s `PUBLISHED` list to publish it. See
the project `CLAUDE.md` (Web Export) for where this sits in the deploy flow.
